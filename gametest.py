...
# 이미지 선언
mole_image = cv2.imread("mole2.png", cv2.IMREAD_UNCHANGED)
    ...
    while cap.isOpened():
        ...
        try:
            ...
            if game_start_event == False:
                cv2.ellipse(
                    image, (w // 2, h // 2 - 50), (72, 90), 0, 0, 360, (0, 0, 255), 0
                )
                cv2.putText(
                    image,
                    "Clap to start a Game",
                    (w // 2 - 300, h // 2 - 85),
                ...
                cv2.putText(
                    image,
                    "Please keep some distance or adjust your webcam",
                    (w // 2 - 210, h // 2 + 130),
                ...
                if get_distance(righthand, lefthand) < 80 and (
                    w // 2 - 100 < nose[0] < w // 2 + 100
                    and h // 2 - 100 < nose[1] < h // 2 + 100
                ):
            ...
            if game_start_event == True and time_remaining > 0:
                ...
                if (
                    rx0 - 50 < righthand[0] < rx0 + 50
                    and ry0 - 50 < righthand[1] < ry0 + 50
                ) or (
                    rx0 - 50 < lefthand[0] < rx0 + 50
                    and ry0 - 50 < lefthand[1] < ry0 + 50
                ):
                ...
                if (
                    rx1 - 50 < righthand[0] < rx1 + 50
                    and ry1 - 50 < righthand[1] < ry1 + 50
                ) or (
                    rx1 - 50 < lefthand[0] < rx1 + 50
                    and ry1 - 50 < lefthand[1] < ry1 + 50
                ):
            ...
            cv2.putText(
                image,
                "Please show your face and keep some distance from your webcam",
                (w // 2 - 260, h // 2 + 220),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                (0, 0, 0),
        ...
        cv2.imshow(
            "Whack-A-Mole Game with Mediapipe Pose",
            cv2.resize(image, None, fx=2.0, fy=2.0),
        )  # 화면크기 2배 키움
